...
class AttentionReadout(nn.Module):
    def __init__(self, out_dim: int = 32, head_dim: int = 16, n_head: int = 4, dino_dim: int = 384):
        super().__init__()
...
class TransformerReadout(nn.Module):
    SHORT_TO_LONG_TOKENS: dict[str, str] = {'patch': 'x_norm_patchtokens', 'cls': 'x_norm_clstoken', 'reg': 'x_norm_regtokens'}

    def __init__(self, out_dim: int = 32, n_head: int = 4, n_layers: int = 2, tokens_used: tuple[str] = ('patch', 'cls', 'reg')):
        super().__init__()

        assert all(token_kind in self.SHORT_TO_LONG_TOKENS for token_kind in tokens_used)

        decoder_layer = nn.TransformerDecoderLayer(d_model=384, nhead=n_head, batch_first=True, norm_first=True)
    ...
    def extra_repr(self) -> str:
        return f"tokens_used={self.tokens_used}"

    def forward(self, dino_output: dict[str, Tensor]) -> Tensor:
        tokens = []
        for token_kind in self.tokens_used:
            token_name = self.SHORT_TO_LONG_TOKENS[token_kind]
            token = dino_output[token_name]
            if token_kind == 'cls':
                token = token.unsqueeze(1)
            tokens.append(token)

        tokens = torch.cat(tokens, dim=1)
        target = dino_output['x_norm_clstoken'].unsqueeze(1)
        decoded = self.transformer_decoder(target, tokens)

        return self.proj(self.norm(decoded)).squeeze(1)
...
class DotProductLoss(nn.Module):
    def forward(self, embeddings: Tensor, labels: Tensor) -> Tensor:
        ...
        weight[torch.eye(gt_similarity.size(0), dtype=bool, device=gt_similarity.device)] = 0
...
class MarginLoss(nn.Module):
    def forward(self, embeddings: Tensor, labels: Tensor) -> Tensor:
        ...
        positive_mask = gt_similarity & ~torch.eye(gt_similarity.size(0), dtype=bool, device=gt_similarity.device)
        negative_mask = ~gt_similarity & ~torch.eye(gt_similarity.size(0), dtype=bool, device=gt_similarity.device)
...
class AccuracyMetric(nn.Module):
    ...
    def forward(self, embeddings: Tensor, labels: Tensor) -> Tensor:
        similarity = self.similarity(embeddings)
        pad_values = torch.full((similarity.size(0), ), fill_value=-float('inf'), dtype=similarity.dtype, device=similarity.device)
        similarity = torch.diagonal_scatter(similarity, pad_values)
        gt_similarity = labels.unsqueeze(0) == labels.unsqueeze(1)

        _values, indices = similarity.topk(10, dim=1)
        is_correct_at_k = gt_similarity[torch.arange(similarity.size(0)).unsqueeze(1), indices]
